...
class VerseBuilder
  ...
  def build
    lines = []
    lines += initial_lines
    lines += chain_lines if has_chain_lines?
    lines.join("\n") + "\n"
  end
  ...
  def chain_lines
    meal_and_reasons.each_cons(2).map do |meal, reason|
      reason ? CHAIN_LINE % [meal.animal, reason.animal_with_qualifier] : meal.comment
    end
  end
